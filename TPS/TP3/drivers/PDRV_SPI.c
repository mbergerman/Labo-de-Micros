...
{
    port_mode[spi_port] = mode; //remember port mode
    chip_selects = ((chip_selects == 0)? 1 : chip_selects); //Minimum number of peripherals is 1
    ...
    if(mode)
    {
        ...
        if (chip_selects > 1 && spi_port != SPI2)
        {
        ...
        if (chip_selects > 2 && spi_port != SPI2)
        {
        ...
        if (chip_selects > 3 && spi_port != SPI2)
        {
        ...
        if (chip_selects > 4 && spi_port == SPI0)
        {
        ...
        if (chip_selects > 5 && spi_port == SPI0)
        {
    ...
    SPI_PTRS[spi_port]->MCR |= SPI_MCR_MSTR(mode); //Define SPI mode Slave or Master
    SPI_PTRS[spi_port]->MCR |= SPI_MCR_PCSIS(PCSIS); //Define SPI chip select inactive status as Active High or Active Low
